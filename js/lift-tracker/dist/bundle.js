import{createClient as Ja}from"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm";var Ee="https://mqfsgammpsumpltfutwl.supabase.co",Le="sb_publishable_thcq6mjdiLYXwzfXUk3pgw_VJWYPFg5";var w=Ja(Ee,Le);function Ce(){let t=window.location.hash.replace(/^#\/?/,"");if(t.startsWith("lift/"))return{name:"detail",liftId:t.slice(5)};if(t==="workout/new")return{name:"workout-new"};let e=t.match(/^workout\/([^/]+)\/edit$/);return e?{name:"workout-edit",workoutId:e[1]}:t==="help"?{name:"help"}:t==="weight"?{name:"weight"}:t==="composite"?{name:"composite"}:t==="history"?{name:"history"}:t==="killstreak"?{name:"killstreak"}:{name:"list"}}function B(){window.location.hash="#/"}function De(t){window.location.hash=`#/lift/${t}`}function _e(){window.location.hash="#/workout/new"}function Te(t){window.location.hash=`#/workout/${t}/edit`}function qe(){window.location.hash="#/help"}function $e(){window.location.hash="#/weight"}function Ae(){window.location.hash="#/composite"}function Me(){window.location.hash="#/history"}function Re(){window.location.hash="#/killstreak"}function At(){window.dispatchEvent(new Event("hashchange"))}async function j(){let{data:t,error:e}=await w.from("lifts").select("*").is("deleted_at",null).order("sort_order",{ascending:!0});if(e)throw e;return t}async function We(t){let{data:e,error:a}=await w.from("lifts").select("*").eq("id",t).maybeSingle();if(a)throw a;return e}async function yt(t,e){let{data:a,error:r}=await w.from("lifts").insert({name:t,sort_order:e}).select().single();if(r)throw r;return a}async function Ie(t,e){let{data:a,error:r}=await w.from("lifts").update({name:e}).eq("id",t).select().single();if(r)throw r;return a}async function Ne(t){let e=t.map((o,n)=>w.from("lifts").update({sort_order:n}).eq("id",o)),r=(await Promise.all(e)).find(o=>o.error);if(r)throw r.error}async function Oe(t){let{error:e}=await w.from("lifts").update({deleted_at:new Date().toISOString()}).eq("id",t);if(e)throw e}async function Ue(t){let{error:e}=await w.from("lifts").update({deleted_at:null}).eq("id",t);if(e)throw e}async function Pe(t){let{data:e,error:a}=await w.from("sets").select("*").eq("lift_id",t).is("deleted_at",null).order("performed_at",{ascending:!0});if(a)throw a;return e}async function Z(t){if(!t||t.length===0)return[];let{data:e,error:a}=await w.from("sets").select("*").in("lift_id",t).is("deleted_at",null).order("performed_at",{ascending:!0});if(a)throw a;return e}async function He(t,e){if(!t||t.length===0)return[];let{data:a,error:r}=await w.from("sets").select("*").in("lift_id",t).is("deleted_at",null).gte("performed_at",e).order("performed_at",{ascending:!0});if(r)throw r;return a}async function tt(t,e,a,r){let{data:o,error:n}=await w.from("sets").insert({lift_id:t,weight:e,reps:a,performed_at:r||new Date().toISOString()}).select().single();if(n)throw n;return o}async function Be(t,e){let{data:a,error:r}=await w.from("sets").update(e).eq("id",t).select().single();if(r)throw r;return a}async function Fe(t){let{error:e}=await w.from("sets").update({deleted_at:new Date().toISOString()}).eq("id",t);if(e)throw e}async function Ve(t){let{error:e}=await w.from("sets").update({deleted_at:null}).eq("id",t);if(e)throw e}async function wt(){let{data:t,error:e}=await w.from("workouts").select("*, workout_lifts(lift_id)").is("deleted_at",null).order("sort_order",{ascending:!0});if(e)throw e;return t.map(a=>({...a,liftIds:(a.workout_lifts||[]).map(r=>r.lift_id)}))}async function Ke(t){let e=t.map((o,n)=>w.from("workouts").update({sort_order:n}).eq("id",o)),r=(await Promise.all(e)).find(o=>o.error);if(r)throw r.error}async function Ye(t){let{data:e,error:a}=await w.from("workouts").select("*, workout_lifts(lift_id)").eq("id",t).maybeSingle();if(a)throw a;return e?{...e,liftIds:(e.workout_lifts||[]).map(r=>r.lift_id)}:null}async function Mt(t,e,a){let{data:r,error:o}=await w.from("workouts").insert({name:t,sort_order:a}).select().single();if(o)throw o;if(e.length>0){let{error:n}=await w.from("workout_lifts").insert(e.map(s=>({workout_id:r.id,lift_id:s})));if(n)throw n}return r}async function Ge(t,e,a){let{error:r}=await w.from("workouts").update({name:e}).eq("id",t);if(r)throw r;let{error:o}=await w.from("workout_lifts").delete().eq("workout_id",t);if(o)throw o;if(a.length>0){let{error:n}=await w.from("workout_lifts").insert(a.map(s=>({workout_id:t,lift_id:s})));if(n)throw n}}async function Xe(t){let{error:e}=await w.from("workouts").update({deleted_at:new Date().toISOString()}).eq("id",t);if(e)throw e}async function je(t){let{error:e}=await w.from("workouts").update({deleted_at:null}).eq("id",t);if(e)throw e}async function bt(){let{data:t,error:e}=await w.from("body_weight").select("*").is("deleted_at",null).order("logged_at",{ascending:!0});if(e)throw e;return t}async function ze(t,e){let{data:a,error:r}=await w.from("body_weight").insert({weight:t,logged_at:e||new Date().toISOString()}).select().single();if(r)throw r;return a}async function Je(t,e){let{data:a,error:r}=await w.from("body_weight").update(e).eq("id",t).select().single();if(r)throw r;return a}async function Qe(t){let{error:e}=await w.from("body_weight").update({deleted_at:new Date().toISOString()}).eq("id",t);if(e)throw e}async function Ze(t){let{error:e}=await w.from("body_weight").update({deleted_at:null}).eq("id",t);if(e)throw e}async function Lt(){let{data:t,error:e}=await w.from("waist_measurements").select("*").is("deleted_at",null).order("logged_at",{ascending:!0});if(e)throw e;return t}async function ta(t,e){let{data:a,error:r}=await w.from("waist_measurements").insert({waist_circumference:t,logged_at:e||new Date().toISOString()}).select().single();if(r)throw r;return a}async function ea(t,e){let{data:a,error:r}=await w.from("waist_measurements").update(e).eq("id",t).select().single();if(r)throw r;return a}async function aa(t){let{error:e}=await w.from("waist_measurements").update({deleted_at:new Date().toISOString()}).eq("id",t);if(e)throw e}async function ra(t){let{error:e}=await w.from("waist_measurements").update({deleted_at:null}).eq("id",t);if(e)throw e}function J(t,e){return t*(1+e/30)}function A(t){let e=new Date(t),a=e.getFullYear(),r=String(e.getMonth()+1).padStart(2,"0"),o=String(e.getDate()).padStart(2,"0");return`${a}-${r}-${o}`}function ft(t){let e=new Map;for(let a of t){let r=A(a.performed_at),o=J(Number(a.weight),Number(a.reps)),n=e.get(r);(!n||o>n.e1rm)&&e.set(r,{date:r,e1rm:o,weight:Number(a.weight),reps:Number(a.reps),setId:a.id})}return Array.from(e.values()).sort((a,r)=>a.date.localeCompare(r.date))}function Rt(t){let e=t.filter(s=>s.dailySeries.length>0);if(e.length===0)return[];let a=new Map;for(let s of e)a.set(s.liftId,s.dailySeries[0].e1rm);let r=new Set;for(let s of e)for(let f of s.dailySeries)r.add(f.date);let o=Array.from(r).sort(),n=[];for(let s of o){let f=0,b=0;for(let m of e){let g=null;for(let y of m.dailySeries)if(y.date<=s)g=y;else break;g&&(f+=g.e1rm/a.get(m.liftId),b+=1)}if(b>0){let m=f/b;n.push({date:s,ratio:m,pct:(m-1)*100})}}return n}function Wt(t,e){if(!e||e.length===0)return!1;let a=Math.max(...e.map(r=>J(Number(r.weight),Number(r.reps))));return t>a}function mt(t){return t.reduce((e,a)=>e+Number(a.weight)*Number(a.reps),0)}function oa(t){let e=Math.round(t);return e===0?"0%":e>0?`+${e}%`:`−${Math.abs(e)}%`}function na(t){let e=new Map;for(let a of t){let r=A(a.performed_at);e.has(r)||e.set(r,[]),e.get(r).push(a)}return Array.from(e.entries()).sort((a,r)=>r[0].localeCompare(a[0]))}function kt(t){let e=new Map;for(let a of t){let r=A(a.logged_at),o=e.get(r);(!o||new Date(a.created_at||0)>=new Date(o.createdAt||0))&&e.set(r,{date:r,weight:Number(a.weight),entryId:a.id,createdAt:a.created_at})}return Array.from(e.values()).sort((a,r)=>a.date.localeCompare(r.date)).map(({date:a,weight:r,entryId:o})=>({date:a,weight:r,entryId:o}))}function sa(t){if(t.length===0)return null;let e=t[0],a=t[t.length-1];return{start:e.weight,current:a.weight,currentDate:a.date,change:a.weight-e.weight}}function Ct(t){let e=new Map;for(let a of t){let r=A(a.logged_at),o=e.get(r);(!o||new Date(a.created_at||0)>=new Date(o.createdAt||0))&&e.set(r,{date:r,waist:Number(a.waist_circumference),entryId:a.id,createdAt:a.created_at})}return Array.from(e.values()).sort((a,r)=>a.date.localeCompare(r.date)).map(({date:a,waist:r,entryId:o})=>({date:a,waist:r,entryId:o}))}var Dt=null,nt=null,st=null,it=null,Ot=14,It="#e8242c",ia="rgba(232, 36, 44, 0.18)",Nt="#f2b134",la="rgba(242, 177, 52, 0.16)",lt="#9a9ca6",dt="rgba(255, 255, 255, 0.08)";function Ut(t,e,{onPointClick:a}={}){Dt&&(Dt.destroy(),Dt=null);let r=e.map(n=>n.date),o=e.map(n=>Math.round(n.pct*10)/10);return Dt=new Chart(t,{type:"line",data:{labels:r,datasets:[{label:"Composite progress",data:o,borderColor:It,backgroundColor:ia,fill:!0,tension:.25,pointRadius:3,pointBackgroundColor:It,pointHitRadius:Ot}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"nearest",intersect:!0},scales:{x:{ticks:{color:lt},grid:{color:dt}},y:{ticks:{color:lt,callback:n=>`${n>0?"+":""}${n}%`},grid:{color:dt}}},plugins:{legend:{display:!1}},onClick:(n,s)=>{s.length&&a&&a(e[s[0].index])}}}),Dt}function da(t,e,{onPointClick:a}={}){nt&&(nt.destroy(),nt=null);let r=e.map(n=>n.date),o=e.map(n=>Math.round(n.e1rm*10)/10);return nt=new Chart(t,{type:"line",data:{labels:r,datasets:[{label:"Estimated 1RM",data:o,borderColor:Nt,backgroundColor:la,fill:!0,tension:.25,pointRadius:4,pointBackgroundColor:Nt,pointHitRadius:Ot}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"nearest",intersect:!0},scales:{x:{ticks:{color:lt},grid:{color:dt}},y:{ticks:{color:lt},grid:{color:dt}}},plugins:{legend:{display:!1}},onClick:(n,s)=>{s.length&&a&&a(e[s[0].index])}}}),nt}function ca(){nt&&(nt.destroy(),nt=null)}function Jt(t,e,{onPointClick:a}={}){st&&(st.destroy(),st=null);let r=e.map(n=>n.date),o=e.map(n=>Math.round(n.weight*10)/10);return st=new Chart(t,{type:"line",data:{labels:r,datasets:[{label:"Weight",data:o,borderColor:It,backgroundColor:ia,fill:!0,tension:.25,pointRadius:3,pointBackgroundColor:It,pointHitRadius:Ot}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"nearest",intersect:!0},scales:{x:{ticks:{color:lt},grid:{color:dt}},y:{ticks:{color:lt},grid:{color:dt}}},plugins:{legend:{display:!1}},onClick:(n,s)=>{s.length&&a&&a(e[s[0].index])}}}),st}function Qt(){st&&(st.destroy(),st=null)}function ua(t,e,{onPointClick:a}={}){it&&(it.destroy(),it=null);let r=e.map(n=>n.date),o=e.map(n=>Math.round(n.waist*10)/10);return it=new Chart(t,{type:"line",data:{labels:r,datasets:[{label:"Waist",data:o,borderColor:Nt,backgroundColor:la,fill:!0,tension:.25,pointRadius:3,pointBackgroundColor:Nt,pointHitRadius:Ot}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"nearest",intersect:!0},scales:{x:{ticks:{color:lt},grid:{color:dt}},y:{ticks:{color:lt},grid:{color:dt}}},plugins:{legend:{display:!1}},onClick:(n,s)=>{s.length&&a&&a(e[s[0].index])}}}),it}function pa(){it&&(it.destroy(),it=null)}function _t(t,{onReorder:e,axis:a="y"}={}){let r=null,o=null,n=0,s=0,f=0,b=0,m=0,g=null,y=null,E=null,p=0,i=0,D=null,S=null;function q(){return Array.from(t.querySelectorAll("[data-reorder-item]"))}function R(c){let l=c.target.closest(".lt-drag-handle");if(!l)return;let x=l.closest("[data-reorder-item]");if(x){if(c.pointerType!=="touch"){c.preventDefault(),C(x,c.clientX,c.clientY);return}if(l.setPointerCapture)try{l.setPointerCapture(c.pointerId),D=l,S=c.pointerId}catch{}E=x,p=c.clientX,i=c.clientY,document.addEventListener("pointermove",k),document.addEventListener("pointerup",L),y=setTimeout(()=>{clearTimeout(y),y=null;let _=E,O=p,N=i;h(),C(_,O,N)},180)}}function I(){if(D&&S!==null&&D.releasePointerCapture)try{D.releasePointerCapture(S)}catch{}D=null,S=null}function h(){clearTimeout(y),y=null,E=null,document.removeEventListener("pointermove",k),document.removeEventListener("pointerup",L)}function k(c){if(!E)return;let l=c.clientX-p,x=c.clientY-i;Math.hypot(l,x)<=10||(h(),I())}function L(){h(),I()}function C(c,l,x){r=c,n=l,s=x,m=x;let _=c.getBoundingClientRect();b=_.top,f=_.left,o=document.createElement(c.tagName),o.className="lt-reorder-placeholder",o.style.height=`${c.offsetHeight}px`,o.style.width=`${c.offsetWidth}px`,c.after(o),c.classList.add("lt-dragging"),c.style.position="fixed",c.style.left=`${_.left}px`,c.style.width=`${_.width}px`,c.style.top=`${b}px`,c.style.zIndex="1000",document.addEventListener("pointermove",X),document.addEventListener("pointerup",at)}function M(){let c=q().filter(_=>_!==r),l=r.getBoundingClientRect(),x=null;if(a==="x"){let _=l.left+l.width/2,O=l.top+l.height/2;for(let N of c){let U=N.getBoundingClientRect(),V=U.left+U.width/2,F=U.top+U.height/2;if(Math.abs(F-O)<U.height/2?_<V:O<F){x=N;break}}}else{let _=l.top+l.height/2;for(let O of c){let N=O.getBoundingClientRect(),U=N.top+N.height/2;if(_<U){x=O;break}}}x?t.insertBefore(o,x):t.appendChild(o)}function P(){let c=m,l=window.innerHeight-m;return c<80?-16*(1-c/80):l<80?16*(1-l/80):0}function T(){if(!r){g=null;return}let c=P();if(c===0){g=null;return}window.scrollBy(0,c),M(),g=requestAnimationFrame(T)}function W(){g===null&&P()!==0&&(g=requestAnimationFrame(T))}function H(){g!==null&&(cancelAnimationFrame(g),g=null)}function X(c){if(r){if(c.preventDefault(),m=c.clientY,a==="x"){let l=c.clientX-n,x=c.clientY-s;r.style.left=`${f+l}px`,r.style.top=`${b+x}px`}else{let l=c.clientY-s;r.style.top=`${b+l}px`}M(),a==="y"&&W()}}function at(){if(!r)return;H(),o.replaceWith(r),r.classList.remove("lt-dragging"),r.style.position="",r.style.left="",r.style.width="",r.style.top="",r.style.zIndex="",document.removeEventListener("pointermove",X),document.removeEventListener("pointerup",at),I();let c=q().map(l=>l.dataset.reorderItem);r=null,o=null,e&&e(c)}t.addEventListener("pointerdown",R)}var Qa="joshuaegage@gmail.com";function fa(){let t=document.activeElement instanceof HTMLElement?document.activeElement:null,e=document.createElement("div");e.className="lt-feedback-overlay",e.innerHTML=`
  ...
  `,t.querySelector("[data-weight-expand]").addEventListener("click",()=>{e&&e()});let r=await bt(),o=kt(r),n=sa(o),s=a&&r.length===0?'<span class="lt-discovery-badge" aria-label="Weight tracker not tried yet">!</span>':"";if(!n){t.innerHTML=`
    ...
    `,t.querySelector("[data-weight-expand]").addEventListener("click",()=>{e&&e()});return}let f=n.change<0?"↘":n.change>0?"↗":"→",b=Tt(ga,!1);function m(){t.classList.toggle("lt-stats-row-expanded",b),b?t.innerHTML=`
        <div class="lt-weight-card-header">
        ...
        <div class="lt-weight-card-body">
          <div class="lt-weight-stats lt-weight-stats-row">
            ...
            <div class="lt-weight-stat">
              <span class="lt-weight-stat-label">Current (${nr(n.currentDate)})</span>
  ...
  `,t.querySelector("[data-back]").addEventListener("click",B);let e=Array.from(t.querySelectorAll("[data-tab]")),a={weight:t.querySelector('[data-tab-panel="weight"]'),waist:t.querySelector('[data-tab-panel="waist"]')},r="weight";e.forEach(c=>{c.addEventListener("click",()=>{c.dataset.tab!==r&&(r=c.dataset.tab,e.forEach(l=>l.setAttribute("aria-selected",String(l===c))),Object.entries(a).forEach(([l,x])=>{x.hidden=l!==r}),r==="weight"?p():W().catch(l=>console.error("[lift-tracker]",l)))})});let o=t.querySelector("[data-weight-form]"),n=t.querySelector("[data-weight-date-input]"),s=t.querySelector("[data-weight-input]"),f=t.querySelector("[data-weight-chart-section]"),b=t.querySelector("[data-weight-canvas]"),m=t.querySelector("[data-weight-empty]"),g=t.querySelector("[data-weight-history]");n.value=A(new Date().toISOString());let y=[];async function E(){y=await bt(),i(),p()}function p(){let c=kt(y);if(c.length===0){f.hidden=!0,m.hidden=!1,Qt();return}f.hidden=!1,m.hidden=!0,a.weight.hidden||Jt(b,c)}function i(){if(y.length===0){g.innerHTML="";return}let c=y.slice().sort((l,x)=>new Date(x.logged_at)-new Date(l.logged_at));g.innerHTML=c.map(l=>`
          <li class="lt-history-row" data-entry-id="${l.id}">
            <button type="button" class="lt-history-main" data-edit-trigger="${l.id}">
              ...
              <span class="lt-history-e1rm">${ya(A(l.logged_at))}</span>
            </button>
          </li>
        `).join(""),g.querySelectorAll("[data-edit-trigger]").forEach(l=>{l.addEventListener("click",()=>D(l.dataset.editTrigger))})}function D(c){let l=g.querySelector(`[data-entry-id="${c}"]`),x=y.find(_=>_.id===c);!l||!x||(l.innerHTML=`
      <form class="lt-edit-set-form" data-edit-form>
        <label>Weight <input type="number" step="0.1" min="0" value="${x.weight}" data-edit-weight /></label>
        <label>Date <input type="date" value="${A(x.logged_at)}" data-edit-date /></label>
    ...
    `,l.querySelector("[data-edit-cancel]").addEventListener("click",i),l.querySelector("[data-edit-delete]").addEventListener("click",async()=>{window.confirm("Delete this weight entry? You'll have a few seconds to undo it after.")&&(await Qe(c),await E(),ct("Weight entry deleted",{onUndo:async()=>{await Ze(c),await E()}}))}),l.querySelector("[data-edit-form]").addEventListener("submit",async _=>{_.preventDefault();let O=Number(l.querySelector("[data-edit-weight]").value),N=l.querySelector("[data-edit-date]").value;if(!(O>=0)||!N)return;let U=new Date(x.logged_at),[V,F,pt]=N.split("-").map(Number);U.setFullYear(V,F-1,pt),await Je(c,{weight:O,logged_at:U.toISOString()}),await E()}))}o.addEventListener("submit",async c=>{c.preventDefault();let l=Number(s.value),x=n.value;if(!(l>=0)||!Number.isFinite(l)||!x)return;let[_,O,N]=x.split("-").map(Number),U=new Date;U.setFullYear(_,O-1,N),await ze(l,U.toISOString()),s.value="",s.focus(),n.value=A(new Date().toISOString()),await E()});let S=t.querySelector("[data-waist-form]"),q=t.querySelector("[data-waist-date-input]"),R=t.querySelector("[data-waist-input]"),I=t.querySelector("[data-waist-chart-section]"),h=t.querySelector("[data-waist-canvas]"),k=t.querySelector("[data-waist-empty]"),L=t.querySelector("[data-waist-history]");q.value=A(new Date().toISOString());let C=[],M=!1,P=null;async function T(){C=await Lt(),M=!0,X(),H()}async function W(){if(M){H();return}P||(k.hidden=!1,k.textContent="Loading waist...",I.hidden=!0,P=T().finally(()=>{P=null})),await P}function H(){let c=Ct(C);if(c.length===0){I.hidden=!0,k.hidden=!1,k.textContent="No waist measurements yet — add your first one above.",pa();return}I.hidden=!1,k.hidden=!0,a.waist.hidden||ua(h,c)}function X(){if(C.length===0){L.innerHTML="";return}let c=C.slice().sort((l,x)=>new Date(x.logged_at)-new Date(l.logged_at));L.innerHTML=c.map(l=>`
          <li class="lt-history-row" data-entry-id="${l.id}">
            <button type="button" class="lt-history-main" data-edit-trigger="${l.id}">
        ...
        `).join(""),L.querySelectorAll("[data-edit-trigger]").forEach(l=>{l.addEventListener("click",()=>at(l.dataset.editTrigger))})}function at(c){let l=L.querySelector(`[data-entry-id="${c}"]`),x=C.find(_=>_.id===c);!l||!x||(l.innerHTML=`
      <form class="lt-edit-set-form" data-edit-form>
        <label>Waist <input type="number" step="0.1" min="0" value="${x.waist_circumference}" data-edit-waist /></label>
        <label>Date <input type="date" value="${A(x.logged_at)}" data-edit-date /></label>
        <div class="lt-edit-actions">
          <button type="submit">Save</button>
          <button type="button" data-edit-cancel>Cancel</button>
  ...
  `;let r=t.querySelector("[data-hamburger-btn]"),o=t.querySelector("[data-header-actions]"),n=240,s=null;function f(d=!0){s&&(clearTimeout(s),s=null),o.classList.remove("lt-header-actions-open"),r.setAttribute("aria-expanded","false"),d&&St(ne,!1),s=setTimeout(()=>{o.hidden=!0,s=null},n)}function b({persist:d=!0,instant:u=!1}={}){s&&(clearTimeout(s),s=null),o.hidden=!1,u?o.classList.add("lt-header-actions-open"):requestAnimationFrame(()=>o.classList.add("lt-header-actions-open")),r.setAttribute("aria-expanded","true"),d&&St(ne,!0)}r.addEventListener("click",()=>{o.hidden?b():f()}),o.addEventListener("click",d=>{d.target.closest("button")&&f()}),Tt(ne,!1)&&b({persist:!1,instant:!0}),t.querySelector("[data-help-btn]").addEventListener("click",qe);let g=t.querySelector("[data-feedback-btn]");g&&g.addEventListener("click",()=>fa()),t.querySelector("[data-logout-btn]").addEventListener("click",()=>w.auth.signOut());let E=t.querySelector("[data-composite-section]"),p=t.querySelector("[data-composite-toggle]"),i=t.querySelector("[data-composite-body]"),D=t.querySelector("[data-chevron]"),S=t.querySelector("[data-composite-summary]"),q=t.querySelector("[data-composite-discovery]");function R(d){p.setAttribute("aria-expanded",String(d)),i.hidden=!d,D.innerHTML=d?"&#9650;":"&#9660;",E.classList.toggle("lt-stats-row-expanded",d)}R(Tt(xa,!0)),p.addEventListener("click",()=>{if(et(z.composite),q.hidden=!0,window.matchMedia("(max-width: 359px)").matches){Ae();return}let d=p.getAttribute("aria-expanded")==="true";R(!d),St(xa,!d)});let I=t.querySelector("[data-killstreak-icon]"),h=t.querySelector("[data-killstreak-label]"),k=t.querySelector("[data-killstreak-sub]"),L=t.querySelector("[data-killstreak-new-badge]");t.querySelector("[data-killstreak-btn]").addEventListener("click",Re);function C(d){let{days:u,tier:v}=Ht(d);I.textContent=v?v.icon:"🎯",h.textContent=v?`${v.label} Killstreak`:"No Killstreak",k.textContent=`${u} Day streak`;let Y=Bt(d).filter(G=>G.track==="rank"),$=Ft(Y,Gt()).length>0;L.hidden=!$}let M=t.querySelector("[data-weight-card]");function P(){et(z.weight),$e()}function T(d){wa(M,{onExpand:P,...d}).catch(u=>{console.error("[lift-tracker]",u),M.classList.remove("lt-stats-row-expanded"),M.innerHTML=`
      ...
      `,M.querySelector("[data-weight-expand]").addEventListener("click",P)})}let W=t.querySelector("[data-history-discovery]");t.querySelector("[data-history-btn]").addEventListener("click",()=>{et(z.history),W.hidden=!0,Me()});let H=t.querySelector("[data-add-lift-form]"),X=t.querySelector("[data-add-lift-toggle]"),at=t.querySelector("[data-add-lift-discovery]"),c=t.querySelector("[data-add-lift-hint]"),l=t.querySelector("[data-create-workout-btn]"),x=t.querySelector("[data-create-workout-discovery]");X.addEventListener("click",()=>{let d=H.hidden;H.hidden=!d,X.setAttribute("aria-pressed",String(d)),X.classList.toggle("lt-add-lift-toggle-active",d),d&&H.querySelector('input[name="name"]').focus()});let _=t.querySelector("[data-lift-list]"),O=t.querySelector("[data-list-empty]");l.addEventListener("click",()=>{l.disabled||_e()});let N=t.querySelector("[data-workout-pills]"),U=t.querySelector("[data-workout-empty-hint]"),V=[],F=re();function pt(){return F&&V.find(d=>d.id===F)||null}function pe(){let d=pt();if(!d)return K;let u=new Set(d.liftIds);return K.filter(v=>u.has(v.id))}function fe(){N.innerHTML=V.map(d=>{let u=d.id===F;return`
        ...
        `}).join("");for(let d of V){let u=N.querySelector(`[data-workout-pill="${d.id}"] [data-workout-pill-name]`);u&&(u.textContent=d.name)}N.querySelectorAll("[data-workout-pill]").forEach(d=>{d.addEventListener("click",()=>{let u=d.dataset.workoutPill;F=F===u?null:u,oe(F),fe(),zt(Et),ye(Et)})}),N.querySelectorAll("[data-workout-edit]").forEach(d=>{d.addEventListener("click",u=>{u.stopPropagation(),Te(d.dataset.workoutEdit)})})}let Xt="lt-fast-mode",me="lt-burst-mode";function Fa(){try{let d=window.localStorage.getItem(Xt);if(d!==null)return d==="true";let u=window.localStorage.getItem(me);return u!==null?(window.localStorage.setItem(Xt,u),window.localStorage.removeItem(me),u==="true"):!1}catch{return!1}}function Va(d){try{window.localStorage.setItem(Xt,String(d))}catch{}}let K=[],rt=Fa(),ot=new Map,Et=[],qt=t.querySelector("[data-mode-toggle]");function he(){qt.textContent=rt?"Normal":"Fast",qt.setAttribute("aria-pressed",String(rt)),qt.classList.toggle("lt-mode-toggle-active",rt)}he(),qt.addEventListener("click",()=>{rt=!rt,Va(rt),he(),zt(Et)}),H.addEventListener("submit",async d=>{d.preventDefault();let u=H.querySelector('input[name="name"]'),v=u.value.trim();if(v){u.value="",u.disabled=!0;try{await yt(v,K.length),await ge()}finally{u.disabled=!1,u.focus()}}}),_t(_,{onReorder:async d=>{let u=[...d],v=new Set(d),Y=K.map($=>v.has($.id)?u.shift():$.id);await Ne(Y),K=Y.map($=>K.find(G=>G.id===$)).filter(Boolean)}}),_t(N,{axis:"x",onReorder:async d=>{await Ke(d),V=d.map(u=>V.find(v=>v.id===u)).filter(Boolean)}});async function ge(){V=await wt(),F&&!V.some($=>$.id===F)&&(F=null,oe(null)),fe(),K=await j();let d=K.length>=2;if(at.hidden=K.length>=2,c.hidden=K.length!==1,l.disabled=!d,l.setAttribute("aria-disabled",String(!d)),x.hidden=!d||V.length>0,U.hidden=!d||V.length>0,K.length===0){_.innerHTML="",O.hidden=!1,O.textContent="Start by adding your first lift above. Once it exists, you can log sets and build workouts around it.",c.hidden=!0,E.hidden=!0,C([]),T({showDiscovery:!1}),W.hidden=!0,q.hidden=!0,ot=new Map,Et=[];return}let u=await Z(K.map($=>$.id)),v=u.length>0;C(u),T({showDiscovery:v&&!Yt(z.weight)}),W.hidden=!v||Yt(z.history),ot=new Map(K.map($=>[$.id,[]]));for(let $ of u){let G=ot.get($.lift_id);G&&G.push($)}let Y=K.map($=>({liftId:$.id,dailySeries:ft(ot.get($.id)||[])}));zt(Y),ye(Y)}function ye(d){let u=pt(),v=u?d.filter($t=>u.liftIds.includes($t.liftId)):d,Y=Rt(v);E.hidden=!1;let $=t.querySelector("[data-composite-canvas]"),G=t.querySelector("[data-composite-empty]"),ht=t.querySelector("[data-composite-scope]"),gt=t.querySelector("[data-composite-blurb]");if(ht.textContent=u?`Measuring ${u.name}`:"Measuring all lifts",gt.textContent=u?"Your average strength gain across the lifts in this workout, relative to where each one started.":"Your average strength gain across all lifts, relative to where each one started.",G.textContent=u?`Log a few sets for lifts in ${u.name} to see this workout's composite progress.`:"Log a few workouts to see your composite progress.",Y.length===0){$.hidden=!0,G.hidden=!1,S.textContent="",q.hidden=!0;return}$.hidden=!1,G.hidden=!0,S.textContent=oa(Y[Y.length-1].pct),q.hidden=Yt(z.composite),Ut($,Y)}function jt(d){let u=ft(ot.get(d)||[]),v=u[u.length-1];return v?`${Math.round(v.e1rm)} lb e1RM`:"No sets yet"}function Ka(d){let u=ot.get(d)||[];return u.length===0?"":u[u.length-1].weight}function zt(d){Et=d;let u=pe();O.hidden=u.length>0,O.textContent=F?"No lifts in this workout yet — tap the pencil above to add some.":"No lifts yet — add your first one above.",_.innerHTML=u.map(v=>rt?`
            <li class="lt-lift-row lt-lift-row-fast" data-reorder-item="${v.id}" data-lift-id="${v.id}">
              ...
              <form class="lt-fast-log" data-fast-log-form="${v.id}">
                <input type="number" inputmode="decimal" step="0.5" min="0" name="weight" placeholder="lb" required value="${Ka(v.id)}" data-fast-weight />
        ...
        `).join("");for(let v of K){let $=_.querySelector(`[data-lift-id="${v.id}"]`)?.querySelector("[data-name-slot]");$&&($.textContent=v.name)}_.querySelectorAll("[data-open-lift]").forEach(v=>{v.addEventListener("click",()=>De(v.dataset.openLift))}),rt&&Ya()}function Ya(){_.querySelectorAll("[data-fast-log-form]").forEach(d=>{let u=d.dataset.fastLogForm;d.addEventListener("submit",async v=>{v.preventDefault();let Y=d.querySelector("[data-fast-weight]"),$=d.querySelector("[data-fast-reps]"),G=d.querySelector("[data-fast-feedback]"),ht=Number(Y.value),gt=Number($.value);if(!(ht>=0)||!Number.isFinite(ht)||!(gt>0)||!Number.isInteger(gt))return;let $t=ot.get(u)||[],Ga=J(ht,gt),be=Wt(Ga,$t),ke=new Date().toISOString(),Xa=await tt(u,ht,gt,ke),ve=[...$t,Xa];ot.set(u,ve),$.value="",$.focus();let Se=_.querySelector(`[data-lift-id="${u}"]`)?.querySelector("[data-last-slot]");Se&&(Se.textContent=jt(u));let ja=A(ke),xe=mt(ve.filter(za=>A(za.performed_at)===ja));G.hidden=!1,G.classList.toggle("lt-pr",be),G.textContent=be?`PR! ${Math.round(xe)} lb today`:`Logged · ${Math.round(xe)} lb today`})})}function we(d){return String(d).replace(/[&<>"']/g,u=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[u])}await ge()}async function La(t,e){let a=await We(e);if(!a||a.deleted_at){t.innerHTML='<p class="lt-empty">Lift not found. <a href="#/">Back to list</a></p>';return}t.innerHTML=`
    <header class="lt-detail-header">
      <button type="button" class="lt-back" data-back aria-label="Back to all lifts">&larr;</button>
  ...
  `,t.querySelector("[data-back]").addEventListener("click",B);let r=t.querySelector("[data-name-input]");r.value=a.name;let o=a.name;r.addEventListener("keydown",h=>{h.key==="Enter"&&r.blur()}),r.addEventListener("blur",async()=>{let h=r.value.trim();if(!h||h===o){r.value=o;return}o=h,await Ie(e,h)}),t.querySelector("[data-delete-lift]").addEventListener("click",async()=>{window.confirm(`Delete "${o}"? You'll have a few seconds to undo it after.`)&&(await Oe(e),B(),ct(`Deleted "${o}"`,{onUndo:async()=>{await Ue(e),At()}}))});let n=Array.from(t.querySelectorAll("[data-tab]")),s={history:t.querySelector('[data-tab-panel="history"]'),details:t.querySelector('[data-tab-panel="details"]')};n.forEach(h=>{h.addEventListener("click",()=>{n.forEach(k=>k.setAttribute("aria-selected",String(k===h))),Object.entries(s).forEach(([k,L])=>{L.hidden=k!==h.dataset.tab}),h.dataset.tab==="details"&&I()})});let f=t.querySelector("[data-log-form]"),b=t.querySelector("[data-weight-input]"),m=t.querySelector("[data-reps-input]"),g=t.querySelector("[data-log-feedback]"),y=[];async function E(){y=await Pe(e)}function p(){if(y.length===0)return;let h=y[y.length-1];b.value=h.weight}f.addEventListener("submit",async h=>{h.preventDefault();let k=Number(b.value),L=Number(m.value);if(!(k>=0)||!Number.isFinite(k)||!(L>0)||!Number.isInteger(L))return;let C=J(k,L),P=Wt(C,y),T=new Date;await tt(e,k,L,T.toISOString()),m.value="",m.focus(),await E(),S(),s.details.hidden||I();let W=A(T.toISOString()),H=mt(y.filter(X=>A(X.performed_at)===W));g.hidden=!1,g.classList.toggle("lt-pr",P),g.textContent=P?`New PR! Today's volume: ${Math.round(H)} lb`:`Logged. Today's volume: ${Math.round(H)} lb`});function i(h){let k=new Map;for(let L of h){let C=A(L.performed_at);k.has(C)||k.set(C,[]),k.get(C).push(L)}return Array.from(k.entries()).sort((L,C)=>C[0].localeCompare(L[0]))}function D(h){let[k,L,C]=h.split("-").map(Number);return new Date(k,L-1,C).toLocaleDateString(void 0,{weekday:"short",month:"short",day:"numeric"})}function S(){let h=s.history;if(y.length===0){h.innerHTML='<p class="lt-empty">No sets logged yet.</p>';return}let k=i(y);h.innerHTML=k.map(([L,C])=>{let M=mt(C),T=C.slice().sort((W,H)=>new Date(H.performed_at)-new Date(W.performed_at)).map(W=>{let H=Math.round(J(Number(W.weight),Number(W.reps)));return`
        ...
        `}).join(""),h.querySelectorAll("[data-edit-trigger]").forEach(L=>{L.addEventListener("click",()=>R(L.dataset.editTrigger))})}function q(h){return s.history.querySelector(`[data-set-id="${h}"]`)}function R(h){let k=q(h),L=y.find(C=>C.id===h);!k||!L||(k.innerHTML=`
      <form class="lt-edit-set-form" data-edit-form>
        ...
        <label>Date <input type="date" value="${A(L.performed_at)}" data-edit-date /></label>
        <div class="lt-edit-actions">
          <button type="submit">Save</button>
          <button type="button" data-edit-cancel>Cancel</button>
          <button type="button" class="lt-delete-set" data-edit-delete>Delete</button>
    ...
    `,k.querySelector("[data-edit-cancel]").addEventListener("click",S),k.querySelector("[data-edit-delete]").addEventListener("click",async()=>{await Fe(h),await E(),S(),s.details.hidden||I(),ct("Set deleted",{onUndo:async()=>{await Ve(h),await E(),S(),s.details.hidden||I()}})}),k.querySelector("[data-edit-form]").addEventListener("submit",async C=>{C.preventDefault();let M=Number(k.querySelector("[data-edit-weight]").value),P=Number(k.querySelector("[data-edit-reps]").value),T=k.querySelector("[data-edit-date]").value;if(!(M>=0)||!(P>0)||!T)return;let W=new Date(L.performed_at),[H,X,at]=T.split("-").map(Number);W.setFullYear(H,X-1,at),await Be(h,{weight:M,reps:P,performed_at:W.toISOString()}),await E(),S(),s.details.hidden||I()}))}function I(){let h=s.details,k=ft(y);if(k.length===0){h.innerHTML='<p class="lt-empty">No sets logged yet.</p>',ca();return}h.innerHTML=`
      <div class="lt-chart-wrap"><canvas data-lift-canvas></canvas></div>
      <p class="lt-point-detail" data-point-detail hidden></p>
    `;let L=h.querySelector("[data-lift-canvas]"),C=h.querySelector("[data-point-detail]");da(L,k,{onPointClick:M=>{C.hidden=!1,C.textContent=`${D(M.date)}: ${M.weight} lb × ${M.reps} (${Math.round(M.e1rm)} e1RM)`}})}await E(),p(),S()}var Ca=60;function Da(t=new Date){let e=new Date(t);return e.setDate(e.getDate()-Ca),e}function ut(t){let e=Math.round(t*10)/10;return e%1===0?String(e):e.toFixed(1)}function se(t,e,a=new Date,r=`last ${Ca} days`,o=[],n=[]){let s=A(a.toISOString()),f=[`Lift Tracker — ${r} (as of ${s})`,""],b=t.filter(m=>(e.get(m.id)||[]).length>0);if(b.length===0)f.push("No sets logged in this period."),f.push("");else{for(let g of b){let y=(e.get(g.id)||[]).slice().sort((i,D)=>new Date(i.performed_at)-new Date(D.performed_at)),E=mt(y),p=Math.max(...y.map(i=>J(Number(i.weight),Number(i.reps))));f.push(g.name);for(let i of y){let D=Math.round(J(Number(i.weight),Number(i.reps)));f.push(`  ${A(i.performed_at)}: ${i.weight} lb x ${i.reps} (e1RM ${D})`)}f.push(`  Sets: ${y.length} | Volume: ${Math.round(E)} lb | Best e1RM: ${Math.round(p)}`),f.push("")}let m=t.length-b.length;m>0&&(f.push(`(${m} lift${m===1?"":"s"} with no sets in this period omitted)`),f.push(""))}if(o.length>0){f.push("Body weight");for(let p of o)f.push(`  ${p.date}: ${ut(p.weight)} lb`);let m=o[0].weight,g=o[o.length-1].weight,y=g-m,E=y>0?"+":"";f.push(`  Start: ${ut(m)} lb | Current: ${ut(g)} lb | Change: ${E}${ut(y)} lb`),f.push("")}if(n.length>0){f.push("Waist");for(let p of n)f.push(`  ${p.date}: ${ut(p.waist)} in`);let m=n[0].waist,g=n[n.length-1].waist,y=g-m,E=y>0?"+":"";f.push(`  Start: ${ut(m)} in | Current: ${ut(g)} in | Change: ${E}${ut(y)} in`),f.push("")}return f.join(`
`).trimEnd()}var sr=[{title:"Weekly killstreak",body:`The banner at the top of the list shows a Call of Duty–style
      ...
      It saves automatically when you tap away or press Enter.`}],ir=`
...
`;async function _a(t){t.innerHTML=`
  ...
  `,t.querySelector("[data-back]").addEventListener("click",B);let e=t.querySelector("[data-export-toggle]"),a=t.querySelector("[data-export-body]"),r=t.querySelector("[data-export-chevron]"),o=t.querySelector("[data-export-textarea]"),n=t.querySelector("[data-export-copy]"),s=t.querySelector("[data-export-status]");e.addEventListener("click",async()=>{let i=!(e.getAttribute("aria-expanded")==="true");if(e.setAttribute("aria-expanded",String(i)),a.hidden=!i,r.innerHTML=i?"&#9650;":"&#9660;",!!i){e.disabled=!0;try{let D=await j(),S=D.map(T=>T.id),q=Da().toISOString(),R=await He(S,q),I=new Map(D.map(T=>[T.id,[]]));for(let T of R){let W=I.get(T.lift_id);W&&W.push(T)}let k=(await bt()).filter(T=>new Date(T.logged_at)>=new Date(q)),L=kt(k),M=(await Lt()).filter(T=>new Date(T.logged_at)>=new Date(q)),P=Ct(M);o.value=se(D,I,new Date,void 0,L,P),s.hidden=!0}finally{e.disabled=!1}}}),n.addEventListener("click",async()=>{o.select();let p=!1;if(navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(o.value),p=!0}catch{p=!1}if(!p)try{p=document.execCommand("copy")}catch{p=!1}s.hidden=!1,s.textContent=p?"Copied!":"Select all (Cmd/Ctrl+A) and copy manually."});let f=t.querySelector("[data-full-export-toggle]"),b=t.querySelector("[data-full-export-body]"),m=t.querySelector("[data-full-export-chevron]"),g=t.querySelector("[data-full-export-textarea]"),y=t.querySelector("[data-full-export-copy]"),E=t.querySelector("[data-full-export-status]");f.addEventListener("click",async()=>{let i=!(f.getAttribute("aria-expanded")==="true");if(f.setAttribute("aria-expanded",String(i)),b.hidden=!i,m.innerHTML=i?"&#9650;":"&#9660;",!!i){f.disabled=!0;try{let D=await j(),S=D.map(C=>C.id),q=await Z(S),R=new Map(D.map(C=>[C.id,[]]));for(let C of q){let M=R.get(C.lift_id);M&&M.push(C)}let I=await bt(),h=kt(I),k=await Lt(),L=Ct(k);g.value=se(D,R,new Date,"all-time",h,L),E.hidden=!0}finally{f.disabled=!1}}}),y.addEventListener("click",async()=>{g.select();let p=!1;if(navigator.clipboard?.writeText)try{await navigator.clipboard.writeText(g.value),p=!0}catch{p=!1}if(!p)try{p=document.execCommand("copy")}catch{p=!1}E.hidden=!1,E.textContent=p?"Copied!":"Select all (Cmd/Ctrl+A) and copy manually."})}async function Ta(t){et(z.composite),t.innerHTML=`
    ...
    <div class="lt-chart-wrap"><canvas data-composite-canvas></canvas></div>
    <p class="lt-empty" data-composite-empty hidden>Log a few workouts to see your composite progress.</p>
  `,t.querySelector("[data-back]").addEventListener("click",B);let[e,a]=await Promise.all([j(),wt()]),r=Sa(a),o=r?e.filter(p=>r.liftIds.includes(p.id)):e,n=o.length?await Z(o.map(p=>p.id)):[],s=new Map(o.map(p=>[p.id,[]]));for(let p of n){let i=s.get(p.lift_id);i&&i.push(p)}let f=o.map(p=>({liftId:p.id,dailySeries:ft(s.get(p.id)||[])})),b=Rt(f),m=t.querySelector("[data-composite-canvas]"),g=t.querySelector("[data-composite-empty]"),y=t.querySelector("[data-composite-scope]"),E=t.querySelector("[data-composite-blurb]");if(y.textContent=r?`Measuring ${r.name}`:"Measuring all lifts",E.textContent=r?"Your average strength gain across the lifts in this workout, relative to where each one started.":"Your average strength gain across all lifts, relative to where each one started.",g.textContent=r?`Log a few sets for lifts in ${r.name} to see this workout's composite progress.`:"Log a few workouts to see your composite progress.",b.length===0){m.hidden=!0,g.hidden=!1;return}m.hidden=!1,g.hidden=!0,Ut(m,b)}function lr(t){let[e,a,r]=t.split("-").map(Number);return new Date(e,a-1,r).toLocaleDateString(void 0,{weekday:"short",month:"short",day:"numeric"})}async function dr(){let t=await j(),e=new Map(t.map(r=>[r.id,r.name]));return(await Z(t.map(r=>r.id))).map(r=>({...r,liftName:e.get(r.lift_id)||"Unknown lift"}))}function cr(t,e){let a=new Map;for(let n of e)a.has(n.liftName)||a.set(n.liftName,[]),a.get(n.liftName).push(n);let r=Array.from(a.entries()).map(([n,s])=>{let b=s.slice().sort((m,g)=>new Date(m.performed_at)-new Date(g.performed_at)).map(m=>{let g=Math.round(J(Number(m.weight),Number(m.reps)));return`
  ...
  `}async function qa(t){et(z.history),t.innerHTML=`
    <header class="lt-detail-header">
      <button type="button" class="lt-back" data-back aria-label="Back to all lifts">&larr;</button>
      <h1 class="lt-weight-view-title">History</h1>
    </header>
    <div class="lt-history-page" data-history-content></div>
  `,t.querySelector("[data-back]").addEventListener("click",B);let e=t.querySelector("[data-history-content]"),a=await dr();if(a.length===0){e.innerHTML=`<p class="lt-empty">No sets logged yet — once you log a set on any lift, it'll show up here.</p>`;return}let r=na(a);e.innerHTML=r.map(([o,n])=>cr(o,n)).join("")}var $a="lt-theme",ie="default";function le(){return Vt($a,ie)}function Aa(t){!t||t===ie?delete document.documentElement.dataset.ltTheme:document.documentElement.dataset.ltTheme=t}function Ma(t){Aa(t),Kt($a,t||ie)}function Ra(){Aa(le())}var ur={rank:"Rank",mastery:"Killstreak Mastery",streak:"Consistency",capstone:"Capstone"},pr=["rank","mastery","streak","capstone"];async function Wa(t){t.innerHTML=`
    <header class="lt-detail-header">
      <button type="button" class="lt-back" data-back aria-label="Back to all lifts">&larr;</button>
  ...
  `,t.querySelector("[data-back]").addEventListener("click",B);let e=await j(),a=e.length?await Z(e.map(i=>i.id)):[],{days:r,tier:o}=Ht(a);t.querySelector("[data-killstreak-current-icon]").textContent=o?o.icon:"🎯",t.querySelector("[data-killstreak-current-label]").textContent=o?`${o.label} Killstreak`:"No Killstreak",t.querySelector("[data-killstreak-current-sub]").textContent=`${r} Day streak`;let n=te(a),s=t.querySelector("[data-killstreak-tier-list]");s.innerHTML=Pt.map(i=>{let D=n[i.key];return`
    ...
    `}).join("");let f=Bt(a),b=f.filter(i=>i.unlocked).length;t.querySelector("[data-achievements-summary]").textContent=`${b} / ${f.length} unlocked. Each badge stays unlocked for good once you've earned it.`;let m=f.filter(i=>i.track==="rank"),g=new Set(Ft(m,Gt()));va(m.filter(i=>i.unlocked).map(i=>i.id));let y=t.querySelector("[data-achievements]");function E(i){if(i.track!=="rank")return`
      ...
      `;let D=i.unlocked&&le()===i.theme.id,S=i.unlocked&&g.has(i.id),q=i.unlocked?`<span class="lt-achievement-card-theme">${i.theme.label} theme${D?" selected":""}</span>`:`<span class="lt-achievement-card-theme lt-achievement-card-theme-locked">🎨 Unlocks: ${i.theme.label}</span>`,R=S?'<span class="lt-achievement-card-new-theme">New theme unlocked</span>':"";return`
      <li class="lt-achievement-card${i.unlocked?" lt-achievement-card-unlocked":" lt-achievement-card-locked"}${S?" lt-achievement-card-new":""}${D?" lt-achievement-card-active-theme":""}">
    ...
    `}function p(){y.innerHTML=pr.map(i=>{let S=f.filter(q=>q.track===i).sort((q,R)=>Number(R.unlocked)-Number(q.unlocked)).map(E).join("");return`
        <section class="lt-achievement-track">
          <h3 class="lt-achievement-track-heading">${ur[i]}</h3>
          ${i==="rank"?'<p class="lt-achievement-track-note">Ranks unlock themes. Try out a theme below.</p>':""}
          <ul class="lt-achievement-list">${S}</ul>
        </section>
  ...
  `,t.querySelector("[data-back]").addEventListener("click",B);let f=t.querySelector("[data-workout-name-input]"),b=t.querySelector("[data-workout-lift-list]"),m=t.querySelector("[data-workout-lifts-empty]"),g=t.querySelector("[data-save-workout]"),y=t.querySelector("[data-workout-save-feedback]");m.hidden=o.length>0;let E=o.filter(S=>s.has(S.id)),p=o.filter(S=>!s.has(S.id));b.innerHTML=[...E.map(i),D(),...p.map(i)].join("");for(let S of o){let R=b.querySelector(`[data-lift-id="${S.id}"]`)?.querySelector("[data-name-slot]");R&&(R.textContent=S.name)}_t(b,{onReorder:()=>{}}),r&&t.querySelector("[data-delete-workout]").addEventListener("click",async()=>{window.confirm(`Delete "${n.name}"? You'll have a few seconds to undo it after.`)&&(await Xe(a),B(),ct(`Deleted "${n.name}"`,{onUndo:async()=>{await je(a),At()}}))}),g.addEventListener("click",async()=>{let S=f.value.trim();if(!S){f.focus();return}let q=Array.from(b.querySelectorAll("[data-reorder-item]")),R=q.findIndex(h=>h.dataset.reorderItem===Ia),I=q.slice(0,R).map(h=>h.dataset.reorderItem);g.disabled=!0,y.hidden=!0;try{if(r)await Ge(a,S,I);else{let h=await wt();await Mt(S,I,h.length)}B()}catch(h){console.error("[lift-tracker]",h),y.hidden=!1,y.textContent="Something went wrong saving the workout.",g.disabled=!1}});function i(S){return`
      <li class="lt-lift-row" data-reorder-item="${S.id}" data-lift-id="${S.id}">
        <div class="lt-lift-row-main lt-lift-row-static">
          <span class="lt-lift-name" data-name-slot></span>
        </div>
        <button type="button" class="lt-drag-handle" aria-label="Drag ${Na(S.name)}">&#8942;&#8942;</button>
      </li>
    `}function D(){return`
    ...
    `}}function Na(t){return String(t).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}var fr=`${window.location.origin}${window.location.pathname}`;function mr(t){return String(t).replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}async function ce(t){let e="signin";function a(o,n,s){return`
      <main class="lt-gate">
        <form class="lt-gate-form" data-auth-form>
          <h1 class="lt-gate-title">Lift Tracker</h1>

          <label for="lt-email">Email</label>
          <input type="email" id="lt-email" name="email" autocomplete="email" required value="${mr(s||"")}">
    ...
    `}function r(o,n,s){t.innerHTML=a(o,n,s),t.querySelector("[data-auth-toggle]").addEventListener("click",()=>{e=e==="signup"?"signin":"signup",r()});let f=t.querySelector("[data-auth-form]");f.addEventListener("submit",async b=>{b.preventDefault();let m=f.email.value.trim(),g=f.password.value,y=f.querySelector('button[type="submit"]');y.disabled=!0,y.textContent=e==="signup"?"Creating account…":"Signing in…";try{let{data:E,error:p}=e==="signup"?await w.auth.signUp({email:m,password:g,options:{emailRedirectTo:fr}}):await w.auth.signInWithPassword({email:m,password:g});if(p)throw p;if(e==="signup"&&!E.session){e="signin",r(null,`Account created. Check ${m} for a confirmation link, then sign in here.`,m);return}}catch(E){r(E.message||"Something went wrong. Try again.",null,m)}})}r()}function Oa(){return new URLSearchParams(window.location.search).get("demo")==="1"}async function Ua(){let{data:t,error:e}=await w.auth.signInAnonymously();if(e)throw e;return await hr(),t}async function hr(){let t=o=>new Date(Date.now()-o*24*60*60*1e3).toISOString(),[e,a,r]=await Promise.all([yt("Bench Press",0),yt("Squat",1),yt("Deadlift",2)]);await Promise.all([tt(e.id,135,8,t(6)),tt(e.id,145,6,t(2)),tt(a.id,185,5,t(5)),tt(a.id,195,5,t(1)),tt(r.id,225,5,t(3))]),await Mt("Full Body",[e.id,a.id,r.id],0)}var Q=document.getElementById("lift-tracker-app");Ra();var Pa=0;async function ue(){let t=++Pa,e=()=>t!==Pa;try{let{data:{session:a}}=await w.auth.getSession();if(e())return;if(!a)if(Oa())try{if(await Ua(),e())return}catch(o){if(e())return;console.error("[lift-tracker] guest demo sign-in failed",o),await ce(Q);return}else return await ce(Q),e(),void 0;let r=Ce();if(r.name==="detail"?await La(Q,r.liftId):r.name==="help"?await _a(Q):r.name==="weight"?await ba(Q):r.name==="composite"?await Ta(Q):r.name==="history"?await qa(Q):r.name==="killstreak"?await Wa(Q):r.name==="workout-new"?await de(Q,{mode:"create"}):r.name==="workout-edit"?await de(Q,{mode:"edit",workoutId:r.workoutId}):await Ea(Q),e())return;window.scrollTo(0,0)}catch(a){if(e())return;console.error("[lift-tracker]",a),Q.innerHTML='<p class="lt-error">Something went wrong loading the lift tracker. Open the console for details.</p>'}}window.addEventListener("hashchange",ue);var Ha=null,Ba=!1;w.auth.onAuthStateChange((t,e)=>{let a=e?.user?.id??null,r=!Ba;Ba=!0;let o=a!==Ha;Ha=a,!(r||!o)&&(B(),ue())});ue();
