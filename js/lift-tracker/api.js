...
export async function createGoals(goals) {
...
export async function createGoalEvent(event) {
  const { data, error } = await supabase
    .from('goal_events')
    .insert(event)
    .select()
...
export async function createGoalEvents(events) {
  const created = [];
  for (const event of events) {
    const row = await createGoalEvent(event);
